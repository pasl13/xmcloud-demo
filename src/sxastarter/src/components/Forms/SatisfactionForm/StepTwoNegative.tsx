...
const StepTwoNegative = ({
  ...
  const createSitecoreItem = async (dateTime: string, answerId: number, opinion: string) => {
    const query = `
      mutation {
        createItem(
          input: {
            ...
            fields: [
              ...
              { name: "MainAnswer", value: "${props.fields.NegativeButton.value}" },
              { name: "SecondaryAnswer", value: "${opinion}" },
