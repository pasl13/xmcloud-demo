...
export const Default = (): JSX.Element => {
  ...
  const [data, setData] = useState<CreateItemResponse | null>(null);
  ...
  const handleCreateItem = async () => {
    ...
    try {
      const response = await client.request<CreateItemResponse>(
        `
      ...
      if (err instanceof Error) {
        ...
        console.error('Erro ao executar a mutation:', err.message);
      } else {
  ...
  return (
    <div>
      ...
      {data && (
        <div>
          <p>
            <strong>ID:</strong> {data.createItem.item.itemId}
          </p>
          <p>
            <strong>Name:</strong> {data.createItem.item.name}
          </p>
          <p>
            <strong>Path:</strong> {data.createItem.item.path}
          </p>
          <h3>Fields:</h3>
          <ul>
            {data.createItem.item.fields.nodes.map((field) => (
              <li key={field.name}>
